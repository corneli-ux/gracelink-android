...
object DatabaseModule {
    ...
    @Provides fun announcementDao(db: GraceDatabase): AnnouncementDao = db.announcementDao()
    @Provides fun churchGroupDao(db: GraceDatabase): ChurchGroupDao = db.churchGroupDao()
    @Provides fun groupMemberDao(db: GraceDatabase): GroupMemberDao = db.groupMemberDao()
    @Provides fun eventRsvpDao(db: GraceDatabase): EventRsvpDao = db.eventRsvpDao()
    @Provides fun leadershipDao(db: GraceDatabase): LeadershipDao = db.leadershipDao()
    @Provides fun ministryDao(db: GraceDatabase): MinistryDao = db.ministryDao()
    @Provides fun groupMessageDao(db: GraceDatabase): GroupMessageDao = db.groupMessageDao()
    @Provides fun directMessageDao(db: GraceDatabase): DirectMessageDao = db.directMessageDao()
    @Provides fun serviceTimeDao(db: GraceDatabase): ServiceTimeDao = db.serviceTimeDao()
    @Provides fun adminNoteDao(db: GraceDatabase): AdminNoteDao = db.adminNoteDao()
    @Provides fun moderationDao(db: GraceDatabase): ModerationDao = db.moderationDao()
}
